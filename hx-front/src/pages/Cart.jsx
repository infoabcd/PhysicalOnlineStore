...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const results = await Promise.all(toFetch.map(async (id) => {
        try {
          const resp = await fetch(`http://localhost:3000/products/${id}`, { credentials: 'include' });
  ...
  const resolveImageSrc = (item) => {
    ...
    return primary.startsWith('http') ? primary : `http://localhost:3000/media/images/${primary}`;
  };
