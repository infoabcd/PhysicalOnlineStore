...
export default function Checkout() {
  ...
  const handlePay = async () => {
    try {
      ...
      const resp = await fetch(`${API_BASE}/orders/create-redirect`, {
        method: 'POST',
        ...
        <div className="grid gap-4 md:grid-cols-[2fr_1fr]">
          ...
          <div className="rounded-lg border border-neutral-700 p-4" style={{ background: '#242424' }}>
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                <label className="block text-sm mb-1">电话(可选)</label>
            ...
            <div className="grid gap-3 md:grid-cols-2 mt-3">
              <div>
                <label className="block text-sm mb-1">邮编(可选)</label>
                    ...
                    {cartItems.map((it)=>{
                      ...
                      return (
                        <div key={`co-${it.commodity_id}`} className="flex items-center gap-3 border-b border-neutral-800 pb-2 last:border-b-0">
                          <img src={it.image_url ? `http://localhost:3000/media/images/${it.image_url}` : '/placeholder.png'} alt={it.title} className="w-10 h-10 object-contain rounded border border-neutral-700" />
