...
function Home() {
  ...
  const fetchData = async () => {
    const url = keyword && keyword.trim()
      ? `http://localhost:3000/search/title/t?title=${encodeURIComponent(keyword.trim())}&page=${currentPage}&limit=${pageSize}`
      : `http://localhost:3000/?page=${currentPage}&limit=${pageSize}`;
            ...
            <div style={{
              ...
              {products.rows?.map(product => (
                <div key={product.id}>
                  <ProductCard
                    title={product.title}
                    imageUrl={product.image_urls ? `http://localhost:3000/media/images/${product.image_urls}` : '/placeholder.png'}
                    price={product.price}
