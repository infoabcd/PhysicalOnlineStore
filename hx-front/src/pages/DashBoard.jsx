...
function Dashboard() {
  ...
  const fetchData = async (page, limit) => {
    ...
    try {
      ...
      // 获取订单统计
      try {
        const statsResp = await fetch('http://localhost:3000/orders/admin/stats', { credentials: 'include' });
        if (statsResp.ok) {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const resp = await fetch('http://localhost:3000/orders/admin', { credentials: 'include' });
      if (resp.ok) {
  ...
  const fetchCategories = async () => {
    setCatLoading(true);
    try {
      const resp = await fetch('http://localhost:3000/admin/categories', { credentials: 'include' });
      if (resp.ok) {
        const arr = await resp.json();
        setCategories(arr);
      }
  ...
  const addCategory = async () => {
    ...
    setCatLoading(true);
    try {
      const resp = await fetch('http://localhost:3000/admin/categories', {
        method: 'POST',
  ...
  const deleteCategory = async (id) => {
    setCatLoading(true);
    try {
      const resp = await fetch(`http://localhost:3000/admin/categories/${id}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      const data = await resp.json();
  ...
  const updateOrderStatus = async (orderNo, status) => {
    try {
      const resp = await fetch(`http://localhost:3000/orders/admin/${orderNo}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
