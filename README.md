...
2. 导入 SQL：
   ...
   ```
3. **数据库连接信息**：修改 `API/config/database.js`。
...
2. 根据环境准备配置：
   - `.env`：参考以下变量：
     ...
     FRONTEND_BASE_URL=http://localhost:5173(你的网站URL)
     ```
   - 或修改 [API/config/config.json]，代码会先尝试从 [.env] 读取，如果失败则使用 [config.json]
...
2. 启动开发服务器：
   ...
   默认 `http://localhost:5173`，会自动请求后端接口。
...
- **配置方式**：
  1. `.env` & 环境变量（推荐，生产使用）：
     ...
     ```
  2. 或修改 [API/config/config.json]（开发用，含 `_comment` 提示）。
  3. 注意：生产环境必须开启 `PAYPAL_ENV=live`，否则无法正常收款。sandbox 仅用于测试收款是否正常，(指定)付款账号由 PayPal 提供。
